...
sous-groupes grâce aux vecteurs colonne des matrices à coefficients entier et en particulier
...
Énonçons d'abord une proposition sur les matrices à coefficients entier qui nous sera forte
utile par la suite.
...
\end{lstlisting}
	...
	La fonction \texttt{hermite\_aux} se repose sur l'algorithme \\d'euclide. En effet, pour
	tout $i < j < m$, on réalise la	division euclidienne de $a_{ij}$ par $a_{ii}$.\\
	Si à la fin de la boucle il existe $j$ tel que $a_{ij} < a_{ii}$, alors on recommence
	...
	tout $j > i$, $a_{ij} = 0$.\\
	De plus, avant de retourner, on réalise la division euclidienne des $a_{ij}$ par
	$a_{ii}$ avec $0 < j < i$.\\
...
\begin{remark}
	...
	le choix ici a été de faire nous même un algorithme à partir de la méthode naïve employée
...
Tout d'abord, la proposition suivante nous permettra de réduire nos forme de matrices dont les
colonnes génèrent un sous groupe de $\Z^2$
...
sur leurs caractéristiques. Nous avons montré dans la section \ref{theoreme_chinois} que nous
...
\begin{proof}
	...
	Nous pouvons donc créer une classe d'équivalence pour la relation $\sim$ où la matrice
...
Grâce aux résultat démontrés dans la section précédente, nous pouvons donc énumérer les\\
...
\begin{proof}
	Par le corollaire \ref{union_m}, on sait que M = $\bigsqcup\limits_{k = 0}^{m} M_k$
	...
	On a $$M_0 =\Mz \text{ avec } A_0 = \Az$$
	Or, pour chaque couple $(a,b) \in A_0$, on $p^b$ matrices différentes.
	De plus, $a \le m - b$. Donc en remplaçant $b$ par $i$, on obtient
	$$ |M_0| = \sum_{i=0}^{m}(m-i)p^i$$
	\\\vspace*{1cm}\\
	Calculons maintenant le cardinal des $M_k$. On a
